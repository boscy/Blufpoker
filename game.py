...
def print_dice(cup): # function for plotting dice in the terminal
    base = '+---------+         +---------+         +---------+'
    sep = '         '
    blank = '|         |'
    left = '| o       |'
...
def make_jpd(options, pk):  # makes a joint probability of the possible rolls, used for calculating bid probabilities
...
class Game:
    def __init__(self, n_players=3, print_info=True, press_to_continue=True, visualize_gui=True, mean_th=3 / 12):
        ...
        self.loser_name = ['L', '0', 'Z', 'E', 'R'] # penalty name for loser
        self.max_penalty = len(self.loser_name)
        self.max_overbid = 3
        self.believe_threshold_mean = mean_th

        # print / visualise info, or looping mode
    ...
    def moveDiceBox(self, player):
        if player == 0:
            self.gui.diceBox.place(rely=0.57, relx=0.1)
        elif player == 1:
            self.gui.diceBox.place(rely=0.02, relx=0.07)
        elif player == 2:
            self.gui.diceBox.place(rely=0.12, relx=0.69)

    def writePenalty(self, player, string):
        penalty = Label(self.gui.penalties, text=string, bg=info_colour)
        if player == 0:
            penalty.grid(row=3, column=3)
        elif player == 1:
            penalty.grid(row=5, column=3)
        elif player == 2:
            penalty.grid(row=7, column=3)
    ...
    def writeKnowledge(self, pw, cb):
        ...
        ax = sns.heatmap(data, annot=self.gui.labels, fmt='', xticklabels=False, yticklabels=False, cmap=c, cbar=False,
        ...
        lbl1 = Label(self.gui.knowledge, text="  Possible worlds higher or equal than current bid  ", bg='green',
                     pady=2, fg='white')
        lbl1.place(rely=0.59, relx=0.05)

        lbl2 = Label(self.gui.knowledge, text="  Possible worlds lower than current bid  ", bg='red', pady=2,
                     fg='white')
    ...
    def bid_possible(self, bid, prints=True):
        if AllPossibleWorlds.index(self.current_bid) < AllPossibleWorlds.index(bid):
            ...
            if self.visualize_gui: self.writeInfo(
                f'Truthful bid is impossible, {bid} not higher than {self.current_bid}')
            return False

    ####################################################################################################################################################
    #########################################                     Calling/Determine Bluff Phase                        #################################
    ####################################################################################################################################################
        ...
        if strategy == 'random':
            ...
            if self.current_bid not in self.players[self.turn].knowledge:
                ...
                jpd = make_jpd(self.players[self.turn].knowledge,
                               self.public_knowledge)  # make a joint probability distribution of the possible worlds given common knowledge

                # determine which of the possible worlds are higher than (or equal to) current bid
                ...
                for w in higher_possible:
                    if w in jpd[0]:
                        # probability if bid is possible is equal to the cumulative probabilities of the bid itself, plus higher possible worlds given common knowledge
                        probability += jpd[1][jpd[0].index(w)]


                if self.visualize_gui: self.writeInfo(
                    "Player 3 decision making process for believing / calling bluff...")
                if self.press_to_continue:
                    input("Press [Enter] to continue...\n")
                if self.print_info: print(
                    f'Probability of rolling possible higher worlds among possible worlds: {round(probability, 3)}')
                if self.visualize_gui: self.writeInfo(
                    f'Probability of rolling possible higher worlds among possible worlds: {round(probability, 3)}')

                # compute variable threshold depending on common knowledge (since belief probability depends on proportion of possible worlds, which is more variable with less common knowledge)
                if len(self.public_knowledge) == 2:
                    ...
                    believe_threshold = np.random.normal(self.believe_threshold_mean, 1 / 12, 1) * 0.75 # less common knowledge -> lower probabilities in JPD, so lower threshold
                else:
                    believe_threshold = np.random.normal(self.believe_threshold_mean, 1 / 12, 1) * 0.5 # less common knowledge -> lower probabilities in JPD, so lower threshold
                ...
                # Finding the correct normal distribution that balances this threshold is hard

                # determine to call bluff of believe by comparing bid probability to believe threshold
                if probability >= believe_threshold:
                    if self.print_info: print(f'Probability of bid: {round(probability, 3)} >= Believe Threshold: {round(believe_threshold[0], 3)}')
                    if self.visualize_gui: self.writeInfo(
                        f'Probability of bid: {round(probability, 3)} >= Believe Threshold: {round(believe_threshold[0], 3)}')
    ...
    def roll_dice(self, roll_strategy):
        # dice are always ordered, thus ranked from highest to lowest value
        # maintain which dice are rolled, such that the common knowledge can be determined
        ...
        if roll_strategy == 'random':
            ...
            rollprint = self.cup.roll_dice_with_value(dicecopy[0][
                                                          2],
                                                      self.visualize_gui)  # always rolls the lowest die first, this always has the highest chance of getting to a higher bet
            if self.visualize_gui: self.writeInfo(rollprint)
            dicecopy[1][2] = 1

            if not self.bid_possible(
                    self.cup.dice,
                    prints=False):  # if the cup is not higher than the bid, make decision whether to roll another die or bluff
                if self.visualize_gui: self.writeInfo(
                    'First roll did not cause for a higher value than the current bid')
                ...
                if dicecopy[0][0] == dicecopy[0][1] and random.randint(0, 1000) < 167:  # first two dice are the same
                    ...
                    if self.visualize_gui: self.writeInfo(
                        'Two open dice are the same, bluffing for poker, since its possible')
                ...
                # otherwise maybe roll, or maybe bluff, depending on the value of the remaining dice
                    ...
                    if self.current_bid.count(6) == 1:  # there is only one 6 in the bid (must be the first)
                        ...
                        if random.randint(1, 1000) > (
                            ...
                            if self.visualize_gui: self.writeInfo(
                                f'[ROLL] Trying to roll {dicecopy[0][1]} to a higher value')
                            rollprint = self.cup.roll_dice_with_value(dicecopy[0][1], self.visualize_gui)
                            if self.visualize_gui: self.writeInfo(rollprint)
                            if self.press_to_continue:
                                input("Press [Enter] to continue...\n")
        ...
        self.public_knowledge = []  # determine common knowledge after rolling
        for i in range(3):
            if dicecopy[1][i] == 0:
                value = dicecopy[0][i]
    ...
    ####################################################################################################################################################
    ########################################                     POKER PHASE (ROLL + PENALISE)                       ###################################
    ####################################################################################################################################################

    def roll_poker(self, threshold):  # poker phase is special, rolling happens openly and is the same for every player
        to_roll = []
        ...
        # Highest dice are equal
            ...
            if self.cup.dice[0] >= threshold:
                ...
                if self.visualize_gui: self.writeInfo(
                    'Equal dice with value NOT high enough, but lowest is. rolling 0, 1 again.')
                to_roll.extend([0, 1])
            else:
        ...
        # Lowest dice are equal
            ...
            if self.cup.dice[1] >= threshold:
                ...
                if self.visualize_gui: self.writeInfo(
                    'Equal dice with value NOT high enough, but highest is. rolling 1, 2 again.')
                to_roll.extend([1, 2])
            else:
                if self.visualize_gui: self.writeInfo('Equal dice with value NOT high enough, rolling all again.')
                to_roll.extend([0, 1, 2])
    ...
    def penalise_poker(self, outcome):  # poker penalty phase
        won = 2
        equal = 1
        lost = 0

        if self.visualize_gui: self.writeInfo(
            f'[PENALISE POKER] The poker was {self.current_bid} and the cup has {self.cup.dice}')
        if outcome == won:
            ...
            if self.visualize_gui: self.writeInfo(
                f'Player {self.turn + 1} has rolled the same poker! No player gets a penalty point.')
        elif outcome == lost:
            self.players[self.turn].penalty_points += 1
            if self.visualize_gui: self.writeInfo(
                f'Player {self.turn + 1} did not roll high enough and gets one penalty point.')
    ...
    def penalise(self):  # non-poker penalty phase
        if self.visualize_gui: self.writeInfo(
            f'[PENALISE] The bid was {self.current_bid} and the cup has {self.cup.dice}')
    ...
    def play(self):
        ...
        while not self.end_game:
            ...
            if self.state == states['start']:  # first turn is different than other turns,
                ...
                if self.visualize_gui: self.writeInfo(
                    f"[STARTING ROLL] Player {self.turn + 1} rolls the dice and rolled {self.cup.dice}")
                if self.visualize_gui: self.drawDice(self.cup.dice)
            ...
            # States below are generally looped through during a turn of a player
            ...
            if self.state == states['roll_dice_phase']:
                ...
                if self.visualize_gui: self.writeInfo(
                    f'Player {self.turn + 1} has rolled the dice and rolled {self.cup.dice}.')
                if self.visualize_gui: self.drawDice(self.cup.dice)
                # if self.print_info: print_dice(self.cup.dice)
                self.update_knowledge()
                if self.press_to_continue:
